#!/bin/sh
#也可以這樣子執行
# Usage: curl -s https://github.com/hugolandteam/script/blob/master/export_language.sh | bash /dev/stdin [<dev|pre|pro>] [useraccount]
display_usage() {
        echo -e "\nUsage:\n$0 [<dev|predev|pre|pro>] [useraccount]\n"
}
...
case "${1,,}" in
        dev)    echo "Dev"
				...
				TARGETPATH2="/www/acc/trunk/application/language"
        ...
        predev)   echo "Dev2"
                TARGETIP="10.117.200.228"
                TARGETPORT=27777
                TARGETPATH="/www/tingzhu/branch/develop/application/language"
				TARGETPATH2="/www/acc/branch/develop/application/language"
                SOURCEIP="10.117.200.227"
                LANGUAGEPATH="/www/zonghoutai/trunk/web/application/export/language"
        ;;
        pre)
				...
				TARGETPATH2="/www/pre/acc/trunk/application/language"
...
ssh ${TARGETUSER}@${TARGETIP} -p${TARGETPORT} "mkdir -p ~/language; sudo tar zcf ~/language/${1,,}_old_language_`date +\%Y\%m\%d_\%H\%M\%S`.tgz -C ${TARGETPATH} . --exclude-vcs && sudo tar zxvf - -C ${TARGETPATH} && sudo tar zxvf - -C ${TARGETPATH2} "